...
def make_plot(data_all , m='' , count = 'temp'):
    ...
    plt.savefig('sources/fermi_pulsar/plots/'+count+'.jpg')
    #plt.show()
...
def source_info(data_fits):
    print(data_fits)
    count = 0 
    for ra,dec in zip(data_fits['ra'] , data_fits['dec']):
        hr()
        ...
        try:
            ...
            data.to_csv('sources/fermi_pulsar/data/'+str(count)+'all.csv')
            data_mean.to_csv('sources/fermi_pulsar/data/'+str(count)+'obs.csv')
        except Exception as e:
            print(e)
...
data_fits = pd.read_csv('../data/pulsars/chandra_fermi_pulsars.csv')
