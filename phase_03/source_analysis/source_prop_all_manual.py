...
def make_plot(data_all , m='' , count = 'temp'):
    ...
    plt.savefig('sources/manual/'+root+'.jpg')
    plt.show()
...
def source_info(RA , DEC , rad , root ):
    count = 0 
    for ra,dec in zip(RA , DEC):
        ...
        try:
            ...
            print(data_mean)
            make_plot(data_mean , master , root)
            hr()
            print('saving data.....')
            data.index.name='index'
            data_mean.index.name='index'
            data.to_csv('sources/manual/'+root+'all.csv')
            data_mean.to_csv('sources/manual/'+root+'obs.csv')
        except Exception as e:
            print(e)
